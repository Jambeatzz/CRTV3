...
def Fterstellung(data):
    ...
    Ft = [[(W - W_0) / (W_inf - W_0) for W in KK1], [(W - W_0) / (W_inf - W_0) for W in KK2], [(W - W_0) / (W_inf - W_0) for W in KK3]]
...
def Ftübereinanderlegung(data):
    ...
    Ft = [[(W - W_0) / (W_inf - W_0) for W in KK1], [(W - W_0) / (W_inf - W_0) for W in KK2], [(W - W_0) / (W_inf - W_0) for W in KK3]]
    Ftref = [[1-(np.e)**(-t/tau[0]) for t in tK1], [1-(np.e)**(-2*t/tau[1])*(1+2*(t/tau[1])) for t in tK1], [1-(np.e)**(-3*t/tau[2])*(1+3*(t/tau[2])+(9/2)*(t/tau[2])**2) for t in tK1]]
...
def numericintegration(Rrest, tK1):
    ...
    Rrestlängen = [
        [(j-i) for i, j in zip(tK1[:-1], tK1[1:])]
    ]
...
def TaukurvenzuBerechnetenvergleiche(data):
    ...
    FtExp = [[1-(np.e)**(-t/5.29665543792108) for t in tK1], [1-(np.e)**(-2*t/10.375577478344564)*(1+2*(t/10.375577478344564)) for t in tK1], [1-(np.e)**(-3*t/15.67981231953802)*(1+3*(t/15.67981231953802)+(9/2)*(t/15.67981231953802)**2) for t in tK1]]
    Ftref = [[1-(np.e)**(-t/tau[0]) for t in tK1], [1-(np.e)**(-2*t/tau[1])*(1+2*(t/tau[1])) for t in tK1], [1-(np.e)**(-3*t/tau[2])*(1+3*(t/tau[2])+(9/2)*(t/tau[2])**2) for t in tK1]]
    
    #plt.figure(figsize=(10, 6))
    for i in range(3):
        plt.figure(figsize=(10, 6)) 
        plt.plot(tK1, Ftref[i], label=f"Referenzkurve Reaktor {i+1}, Tau = {tau[i]}")
        plt.plot(tK1, FtExp[i], label=f"Referenzkurve mit Berechnetem Tau {i+1}, Tau = {tauexp[i]}")
        plt.xlabel("Zeit [min]")
        plt.ylabel("F(t)")
        plt.title("Ber. F(t)-Kurven der Reaktor-Kaskade, Vergleich mit Experimentell ermitteltem Tau")
        plt.legend()
        plt.grid(True)
        plt.ylim(-0.1, 1.1) # Y-Achse von -0.1 bis 1.1 für bessere Sichtbarkeit von 0 und 1
...
def TauKurvenzuexpKurvenvergleiche(data):
    ...
    for i in range(3):
        ...
        plt.xlabel("Zeit [min]")
        plt.ylabel("F(t)")
        plt.title("F(t)-Kurven der Reaktor-Kaskade, Vergleich mit Experimentell ermitteltem Tau")
        plt.legend()
        plt.grid(True)
        plt.ylim(-0.1, 1.1) # Y-Achse von -0.1 bis 1.1 für bessere Sichtbarkeit von 0 und 1
